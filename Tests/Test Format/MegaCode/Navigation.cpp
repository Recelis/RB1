...
//  Serial.println(remoteDirection);
  ...
  String v = String(xdirection) + " " + String(ydirection);
//  Serial.println(v);
  int direction = atan2(ydirection, xdirection) / PI * 180;
  int spin = 0;
  if (direction < 90 && direction > 0) spin = 5;
  else if (direction > 90 & direction > 0 && direction < 180) spin = -5;
  else spin = 0;
  static int vectoredData [4];
  vectoredData[0] = 1; // placeholder value
  vectoredData[1] = remoteSpeed;
